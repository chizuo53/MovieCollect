...
class MovieUpdater:
    ...
    def _get_general_spidercls(self):
        ...
        spider_module_pre, spider_module, spider_modulename = path.rsplit('.', 2)
        if spider_module not in sys.modules:
            module = importlib.import_module(spider_module_pre+'.'+spider_module)
        else:
            importlib.reload(sys.modules[spider_module])
            module = sys.modules[spider_module]
        spidercls = getattr(module, spider_modulename)
        assert (
            inspect.isclass(spidercls)
            and issubclass(spidercls, Spider)
        )
        spidercls.name = self.settings.get('GENERAL_SPIDER_NAME', '_general_spider')
        return spidercls
